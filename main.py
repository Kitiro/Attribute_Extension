...
def train_and_test(loader, dataset, args):
    ...
    postfix = "C_W-{}_GenAttr-{}".format(c_w, args.attr_num)
    if args.screen:
        postfix += "_screen"
    params = list(model.parameters()) + list(center_loss.parameters())  
    # joint learning with center loss
    opt = torch.optim.Adam(
        params, lr=lr, weight_decay=1e-3
    ...
    log_path = os.path.join(args.output, args.dataset, "LOG", f"Log_{postfix}_Seed_{args.seed}.log")
    ...
    with torch.set_grad_enabled(True):
        for epoch in range(args.epochs):
            ...
            for visual_batch, attr_batch, label_batch in loader:
                ...
                loss_center = c_w * center_loss(out_visual, label_batch.cuda())
